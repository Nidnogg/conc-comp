...
void startWrite(int tid, int writer_i) {
    pthread_mutex_lock(&mutex);
    ...
    if(reading > 0 || writing > 0 || waitingToRead > waitingToWrite) {
        ...
        for(; writer_i < NTHREADS_WRITE; writer_i++) {
...
void endWrite(int tid) {
    ...
    if(waitingToWrite) pthread_cond_broadcast(&cond_write);
...
void * writer (void *arg) {
    ...
    while(1) {
        ...
        startWrite(tid, writer_i); //!! tranca outros escritores/leitores
        writer_i = 0;
