...
void startRead(int tid) {
    ...
    while(writing > 0) { 
        printf("Thread Leitora de ID %d irá se bloquear esperando writing > 0 (writing == %d)\n", tid, writing);
...
void startWrite(int tid) {
    ...
    while((reading > 0 || writing > 0)) {
        printf("Thread Escritora de ID %d irá esperar cond_write (pois reading (%d) > 0 ou writing (%d) > 0 )\n", tid, reading, writing);
